...
const LoginPage = () => {
    ...
    const handleSubmit = async(event: any) => {
        ...
        // Login the user
        usersService.loginUser(userName, password).then((response) => {
            localStorage.setItem("token", response.token);
            navigate("/");
        }).catch((error) => {
            setModalMessage(`Some error occurred during the login!`);
            setShowModal(true);  
        })
    };
    ...
    return (
        <div id="lp-login-container">
            <div id="lp-welcome-panel">
                <h1>WELCOME TO</h1>
                <Figure>
                    <Figure.Image
                        width={250}
            ...
            <Card id="lp-login-card">
                <Card.Body>
                    <Form noValidate onSubmit={handleSubmit}>
                        ...
                        <div id="lp-login-card-button-panel">
                             <Button type="submit" id="login-button" variant="dark">Login</Button>
                        </div>
                        <div id="lp-register-notification-panel">
