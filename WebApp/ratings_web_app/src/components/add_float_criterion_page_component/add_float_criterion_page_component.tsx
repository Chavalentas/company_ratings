...
import "./add_float_criterion_page_component.css";
...
const AddFloatCriterionPage = () => {
    ...
    const handleSubmit = async(event: any) => {
        ...
        criteriaService.registerFloatCriterion(criterionToRegister)
        .then((response) => {
    ...
    const createCriterion = () : FloatCriterion => {
        ...
        return new FloatCriterion(0, clientId, criterionName, minValue, maxValue, worstValue, bestValue);
    }
    
    return (
        <div id="afcp-add-criterion-container">
            <div id="afcp-welcome-panel">
                <h1>WELCOME TO</h1>
                <Figure>
                    <Figure.Image
                        width={250}
                ...
                <h3>{clientName}</h3>
                <h3>New continuous criterion</h3>
            </div>
            <Card id="afcp-add-criterion-card">
                <Card.Body>
                    <Form noValidate onSubmit={handleSubmit}>
                        ...
                        <Form.Group className="mb-3 afcp-group-horizontal" controlId="addForm.minValueInput">
                        <div className="afcp-group-horizontal-member afcp-group-horizontal-left">
                                <Form.Label>Min value (Current: {minValue})</Form.Label>
                                <Form.Range value={minValue} step={criteriaConfig.floatStepSize}  onChange={(e) => handleMinValueChange(Number(e.target.value))} min={criteriaConfig.numericSmallestPossible} max={criteriaConfig.numericLargestPossible}/>
                                <div className="afcp-axis-indicator-div">
                                    <div className="afcp-axis-indicator-left">
                                        <h5>{criteriaConfig.numericSmallestPossible}</h5>
                                    </div>
                                    <div className="afcp-axis-indicator-right">
                                        <h5>{criteriaConfig.numericLargestPossible}</h5>
                                    </div>
                                </div>
                                <span className="afcp-error-background">{minValueError}</span>
                            </div>
                            <div className="afcp-group-horizontal-member afcp-group-horizontal-right">
                        ...
                        <Form.Group className="mb-3 afcp-group-horizontal" controlId="addForm.maxValueInput">
                           <div className="afcp-group-horizontal-member afcp-group-horizontal-left">
                                <Form.Label>Max value (Current: {maxValue})</Form.Label>
                                <Form.Range value={maxValue} step={criteriaConfig.floatStepSize} onChange={(e) => handleMaxValueChange(Number(e.target.value))} min={criteriaConfig.numericSmallestPossible} max={criteriaConfig.numericLargestPossible} />
                                <div className="afcp-axis-indicator-div">
                                    <div className="afcp-axis-indicator-left">
                                        <h5>{criteriaConfig.numericSmallestPossible}</h5>
                                    </div>
                                ...
                                <span className="afcp-error-background">{maxValueError}</span>
                            </div>
                            <div className="afcp-group-horizontal-member afcp-group-horizontal-right">
                        ...
                        <div id="afcp-add-criterion-card-button-panel">
                            <Button className="afcp-add-criterion-card-button-panel-button" variant="dark"  onClick={() => {navigate(-1)}}>Back</Button>
                            <Button type="submit" className="afcp-add-criterion-card-button-panel-button" variant="dark">Submit</Button>
                        </div>
                    </Form>
...
export default AddFloatCriterionPage;